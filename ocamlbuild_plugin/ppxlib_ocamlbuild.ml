...
let prog_of_libs ~native libs =
  "ppxlib-" ^ String.concat "+" libs -.- exe_ext ~native
...
let dispatch = function
  | After_rules -> begin
      ...
      pflag [ "ocaml"; "compile"  ] "ppxlib" (pp_command_of_string ~native);
      pflag [ "ocaml"; "ocamldep" ] "ppxlib" (pp_command_of_string ~native);
      pflag [ "ocaml"; "doc"      ] "ppxlib" (pp_command_of_string ~native);

      pdep [ "ocaml"; "compile"  ] "ppxlib" (deps_of_string ~native);
      pdep [ "ocaml"; "ocamldep" ] "ppxlib" (deps_of_string ~native);
      pdep [ "ocaml"; "doc"      ] "ppxlib" (deps_of_string ~native);

      let target = "ppxlib-%" -.- exe_ext ~native in
